...
ParsingContext *parse_context_default_create() {
    ...
    if (err.type != ERROR_NONE) {
        printf("ERROR: Failed to set builtin integer type in types "
               "environment.\n");
    }
    return ctx;
}
...
int parse_integer(Token *token, Node *node) {
    ...
    } else if ((node->value.integer = strtoll(token->beginning, &end, 10)) !=
               0) {
        if (end != token->end) {
            return 0;
        }
        ...
        if (parse_integer(&current_token, working_result)) {
            ...
            if (strcmp("defun", symbol->value.symbol) == 0) {
                ...
                if (expected.found) {
                    ...
                    int status = environment_set(context->variables,
                                                 symbol_for_env, type_symbol);
                    if (status != 1) {
                    ...
                    // Symbol is not `defun` and it is not followed by an
                    // assignment operator `:`.
                    ...
                    if (expected.found) {
                        ...
                        // IG, and then start parsing function argument
                        // expressions.
                        ...
                        // TODO: Check if it's a variable access (defined
                        // variable)
                    }
                }
